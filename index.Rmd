---
title: "Module 2: Do You Trust Me?"
subtitle: "EME6665: Synthesis, Analysis, & Argumentation"
author: "Dr. Bret Staudt Willet"
date: "September 7, 2022"
output:
  xaringan::moon_reader:
    lib_dir: libs
    seal: true
    css: [default, 'custom.css', 'custom-fonts.css']
    nature:
      titleSlideClass: [left, bottom]
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
---

```{r setup, include=FALSE}
usethis::use_git_ignore(c("*.csv", "*.rds"))
options(htmltools.dir.version = FALSE)

library(knitr)
library(tidyverse)
library(xaringan)
library(fontawesome)
```

class: inverse, center, middle

# `r fa("fas fa-images", fill = "#fff")`

**View the slides:** 

[bretsw.com/eme6665-fs22-module2](https://bretsw.com/eme6665-fs22-module2)

---

class: inverse, center, middle

# `r fa("comments", fill = "#fff")` <br><br> Module 1 Discussion <br> Themes

---

# `r fa("comments", fill = "#fff")` Keep Calm

```{r, out.width = "720px", echo = FALSE, fig.align = "center"}
include_graphics("img/calm.jpg")
```

---

# `r fa("comments", fill = "#fff")` Get Organized

```{r, out.width = "720px", echo = FALSE, fig.align = "center"}
include_graphics("img/tidy.jpg")
```

---

# `r fa("comments", fill = "#fff")` Topic Sentences

```{r, out.width = "720px", echo = FALSE, fig.align = "center"}
include_graphics("img/start.jpg")
```

---

# `r fa("comments", fill = "#fff")` Theory Spotlight

```{r, out.width = "720px", echo = FALSE, fig.align = "center"}
include_graphics("img/streetlamp.jpg")
```

---

# `r fa("comments", fill = "#fff")` Writing Titles

```{r, out.width = "720px", echo = FALSE, fig.align = "center"}
include_graphics("img/pen.jpg")
```

---

class: inverse, center, middle

# `r fa("fas fa-thumbtack", fill = "#fff")` <br><br> Do You Trust Me?

---

# `r fa("comments", fill = "#fff")` News Literacy

```{r, out.width = "720px", echo = FALSE, fig.align = "center"}
include_graphics("img/2-news-literate.png")
```

<div class="caption">
<p><a href="https://newslit.org/tips-tools/how-news-literate-are-you-quiz/" target="_blank">How News Literate Are You? (quiz)</a></p>
</div>

---

class: inverse, center, middle

# `r fa("book-open", fill = "#fff")` <br><br> Module 2 readings

---

# `r fa("book-open", fill = "#fff")` Module 2 readings

```{r, out.width = "500px", echo = FALSE, fig.align = "center"}
include_graphics("img/reading.jpg")
```

--

- 2.1 Searching and Screening: The Practical Screen and Methodological Quality (Fink, 2020, Chapter 2)

--

- 2.2 A Guide for the Perplexed: Scientific Educational Research, Methodolatry, and the Gold Versus Platinum Standards (Phillips, 2006)

--

- 2.3 Standards for Reporting on Empirical Social Science Research in AERA Publications (AERA, 2006)

---

# `r fa("book-open", fill = "#fff")` Module 2 readings

```{r, out.width = "500px", echo = FALSE, fig.align = "center"}
include_graphics("img/reading.jpg")
```

- 2.4 Preferred Reporting Items for Systematic Reviews and Meta-Analyses: The PRISMA Statement (Moher et al., 2009)

--

- 2.5 Determining Validity in Qualitative Inquiry (Creswell & Miller, 2000)

--

- 2.6 Collaboration, Multiple Methods, Trustworthiness: Issues Arising from the 2014 International Conference on Self-study of Teacher Education Practices (Mena & Russell, 2017)

---

# PRISMA Standards

```{r, out.width = "600px", echo = FALSE, fig.align = "center"}
include_graphics("img/2-prisma.png")
```

<div class="caption">
<p>(Moher et al., 2009)</p>
</div>

---

# Creswell & Miller (2000)

```{r, out.width = "100%", echo = FALSE, fig.align = "center"}
include_graphics("img/2-creswell-miller.png")
```

<div class="caption">
<p>(Creswell & Miller, 2000)</p>
</div>

---

class: inverse, center, middle

# `r fa("fas fa-binoculars", fill = "#fff")` <br><br> Looking ahead

---

# `r fa("fas fa-calendar-day", fill = "#fff")` Semester schedule

- **Module 1:** Threads of Chapter One

- **Module 2: Do You Trust Me?**

- **Module 3:** A Handbook's Tale

- **Module 4:** Systematic Not Automatic

- **Module 5:** Synthesis Over Summary

- **Module 6:** Elements of Style (for Academic Writing)

- **Module 7:** Weaving Together Chapter One

---

# `r fa("fas fa-calendar-day", fill = "#fff")` Module structure

```{r, out.width = "540px", echo = FALSE, fig.align = "center"}
include_graphics("img/workshop.jpg")
```

- `r fa("video", fill = "#782F40")` Meet (Wednesdays at 7pm EST on Zoom)

- `r fa("book-open", fill = "#782F40")` Read

- `r fa("comments", fill = "#782F40")` Discuss

- `r fa("fas fa-keyboard", fill = "#782F40")` Create

---

# `r fa("fas fa-calendar-day", fill = "#fff")` Major Assignments

```{r, out.width = "260px", echo = FALSE, fig.align = "center"}
include_graphics("img/build.jpg")
```

**Assignments** (70%)

- Module 1 Assignment: Research Map (100 points)

- **Module 2 Assignment: Article Critique (100 points)**

- Module 3 Assignment: Topic Reflection (100 points)

- Module 4 Assignment: Analytic Memo (100 points)

- Module 5 Assignment: One Completed Subsection (100 points)

- Module 6 Assignment: Peer Review (50 points)

- Module 7 Assignment: Chapter One Draft One (150 points)

---

# `r fa("fas fa-calendar-day", fill = "#fff")` Article Critique Assignment

--

- Topic

--

- Problem

--

- Purpose

--

- Framework

--

- Literature Review

--

- Research Questions

--

- Methods

--

<hr>

### Your main objective is to assess the **clarity** and **effectiveness** of each area

---

class: inverse, center, middle

# `r fa("fas fa-thumbtack", fill = "#fff")` Practice Drawing the Map

<div class="padlet-embed" style="border:1px solid rgba(0,0,0,0.1);border-radius:2px;box-sizing:border-box;overflow:hidden;position:relative;width:100%;background:#F4F4F4"><p style="padding:0;margin:0"><iframe src="https://fsu.padlet.org/embed/jbwz0hokd7llz8bf" frameborder="0" allow="camera;microphone;geolocation" style="width:100%;height:480px;display:block;padding:0;margin:0"></iframe></p><div style="display:flex;align-items:center;justify-content:end;margin:0;height:28px"><a href="https://padlet.com?ref=embed" style="display:block;flex-grow:0;margin:0;border:none;padding:0;text-decoration:none" target="_blank"><div style="display:flex;align-items:center;"><img src="https://padlet.net/embeds/made_with_padlet_2022.png" width="114" height="28" style="padding:0;margin:0;background:0 0;border:none;box-shadow:none" alt="Made with Padlet"></div></a></div></div>

<div class="caption">
<p><a href="https://rgbstudios.org/dnd-dice/" target="_blank">D&D Dice: D6 + D8</a></p>
</div>



---

class: inverse, center, middle

# `r fa("fas fa-question", fill = "#fff")` <br><br> Questions

<hr>

**What questions can I answer for you now?**

**How can I support you this week?**

<hr>

`r fa("envelope", fill = "#fff")` [bret.staudtwillet@fsu.edu](mailto:bret.staudtwillet@fsu.edu) | `r fa("twitter", fill = "#fff")` [@bretsw](https://twitter.com/bretsw) | `r fa("globe", fill = "#fff")` [bretsw.com](https://bretsw.com) | `r fa("fab fa-github", fill = "#fff")` [GitHub](https://github.com/bretsw/)